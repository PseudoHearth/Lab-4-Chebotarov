...
Money Money::operator+(const Money& D)
{
    Money tmp;
    int k = 0;
    if ((this->Kopiyka + D.Kopiyka) >= 100)
    {
        tmp.Kopiyka = (this->Kopiyka + D.Kopiyka) - 100;
        k = 1;
    }
    else
        tmp.Kopiyka = this->Kopiyka + D.Kopiyka;
    tmp.Gryvnya = this->Gryvnya + D.Gryvnya + k;
    cout << "Resultat dodavanya: ";
    return tmp;
}
